...
def replace_day(day_desc_list, d):
    pattern = re.compile(rf"mc\.{d}.*")
    for line in day_desc_list:
...
def replace_service(day_desc_list, s, tr):
...
def replace_key(key_desc_list, hymn_name, service_id):
    pattern = re.compile(rf"^tr{service_id}.*\.{hymn_name}\..*")
    for line in key_desc_list:
        if pattern.match(line):
            if ":" in line:
                return line.split(":")[-1].strip().strip('"')
            else:
...
def main():
    # Paths - Keeping original for Greek Triodion
    ...
    output_path = "C:/git/ages-alwb-system/net.ages.liturgical.workbench.system/MEDIA_INDEX_UTILITY/output/grTR.html"
    ...
    # 1. Load External Data (Snippet mode: no header_footer)
    ...
    with open(media_map_path, 'r', encoding='utf-8') as mm:
        for line in mm:
            ...
            # Parsing: tr.d035.trMA.Exaposteilarion1.c12
            try:
                current_line[0] = "Triodion"
                day_code = str_line[3:7]      # d035
                service_id = str_line[10:12]  # MA
                dot_after_hymn = str_line.find(".", 13)
                hymn_name = str_line[13:dot_after_hymn]

                # Transformations
                current_line[1] = replace_day(day_desc, day_code)
                current_line[2] = replace_service(day_desc, service_id, "")
                current_line[3] = replace_key(key_desc, hymn_name, service_id)
            except: continue
    ...
    with open(output_path, 'w', encoding='utf-8') as out:
        out.write(fs)
    print("Successfully generated grTR.html snippet.")

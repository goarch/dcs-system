...
def replace_day(d, m):
    if re.match(r".\d\d", d):
        try:
            day_num = int(d[1:])
            return f"{m[:3]} {day_num}"
        except:
            return m
    else:
        # Special commemorations logic
        if d == "dHF" and m == "July": return "Jul 13-19: Sunday of the Holy Fathers"
        elif d == "dHF" and m == "October": return "Oct 11-17: Sunday of the Holy Fathers"
        elif d == "dFF": return "Dec 11-17: Sunday of the Forefathers"
        elif d == "dBC": return "Dec 18-24: Sunday Before Christmas"
        elif d == "dAC": return "Sunday After Christmas"
        return m
...
def replace_key(arr, str_val, id_val):
    pattern = re.compile(rf"^me{id_val}.*\.{str_val}\..*")
    for line in arr:
        if pattern.match(line):
            if ":" in line:
                parts = line.split(":")
...
def main():
    # Paths - Keeping original paths for Greek Menaion
    ...
    output_path = "C:/git/ages-alwb-system/net.ages.liturgical.workbench.system/MEDIA_INDEX_UTILITY/output/grME.html"
    ...
    # 1. Load External Data (Headers/Footers removed)
    ...
    with open(media_map_path, 'r', encoding='utf-8') as mm:
        for line in mm:
            ...
            try:
                ...
                dot_after_hymn = str_line.find(".", 16)
                hymn_name = str_line[16:dot_after_hymn]
                ...
                current_line[4] = replace_key(key_desc, hymn_name, service_id)
            except: continue

            # Music Style and Path
            music_style = ""
            ...
            # Compare hierarchy for changes
            for i in range(len(current_line)):
                if current_line[i] != previous_line[i]:
                    # Close necessary levels
    ...
    with open(output_path, 'w', encoding='utf-8') as out:
        out.write(fs)
    print("Successfully generated grME.html snippet.")

...
def tabs(n):
    return "    " * n # Standardized 4-space indentation

def replace_day(arr, d):
    # Matches "mc.dXXX" and extracts the descriptive title
    pattern = re.compile(rf"^mc\.{re.escape(d)}.*")
    for line in arr:
        if pattern.match(line):
            try:
                # Extracting title after 'mc.d000 = "'
...
def replace_service(arr, s, tr):
    # Lookup for service links (e.g., ma.html.link)
...
def replace_key(arr, str_val, id_val):
    # Key descriptor lookup for specific hymns
    pattern = re.compile(rf"^tr{id_val}.*\.{str_val}\..*")
    for line in arr:
        if pattern.match(line):
            if ":" in line:
                parts = line.split(":")
...
def main():
    ...
    output_path = "C:/git/ages-alwb-system/net.ages.liturgical.workbench.system/MEDIA_INDEX_UTILITY/output/enTR.html"
    ...
    with open(media_map_path, 'r', encoding='utf-8') as mm:
        for line in mm:
            ...
            try:
                ...
                current_line[2] = replace_service(day_desc, service_id, special_tr)
    ...
    with open(output_path, 'w', encoding='utf-8') as out:
        out.write(fs)
    print("Successfully generated enTR.html snippet.")
